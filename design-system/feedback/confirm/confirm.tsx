...
export const Confirm = ({ className, name, children }: ConfirmProps) => {
    ...
    return (
        <confirmContext.Provider value={name}>
            <Dialog open={!!t?.open}>
                <DialogContent className={cn('w-[480px] gap-0 px-4', className)}>{content}</DialogContent>
            </Dialog>
...
export const CancelButton = ({
    ...
    name,
    children
}: PropsWithChildren<{ className?: string; name?: string; variant?: 'fill-primary' | 'secondary' }>) => {
    const onConfirm = useConfirm((s) => s.onConfirm);
    const contextName = useConfirmContext();

    return (
        <Button className={cn('h-11 min-w-[100px] rounded-lg px-2 focus:outline-none', className)} onClick={() => onConfirm(name || contextName, false)}>
            {children}
        </Button>
    );
};
