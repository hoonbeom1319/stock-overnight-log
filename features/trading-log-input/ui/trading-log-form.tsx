...
export function TradingLogForm({
    ...
    return (
        <Card className="w-full space-y-5">
            ...
            <form onSubmit={onSearch} className="grid grid-cols-1 gap-4">
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                    ...
                    <Popover.Root open={shouldShowSuggestions} modal={false}>
                        ...
                        <Popover.Portal>
                            <Popover.Content
                                align="start"
                                sideOffset={8}
                                className="z-20 w-(--radix-popover-trigger-width) overflow-hidden rounded-lg border border-slate-700 bg-slate-900 shadow-xl outline-none"
                ...
                <div className="flex flex-col gap-3 pt-1">
                    <Button type="submit" disabled={!canSearch || isFetching} className="min-h-11 w-full">
                        {isFetching ? '조회 중...' : '조회'}
                    </Button>
                    <Button type="button" variant="secondary" onClick={onSave} disabled={!preview || isSaving} className="min-h-11 w-full">
                        {isSaving ? '저장 중...' : '저장'}
                    </Button>
                </div>
            </form>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4">
                ...
                {isError ? (
                    ...
                    <div className="mt-3 space-y-2 text-sm">
                        <div className="flex min-w-0 items-baseline justify-between gap-3 rounded-lg bg-slate-900 p-3">
                            <p className="shrink-0 text-slate-400">당일 종가(매수가)</p>
                            <p className="min-w-0 flex-1 text-right text-lg font-semibold whitespace-nowrap tabular-nums text-slate-100">
                                {preview.buyPrice.toLocaleString()}원
                            </p>
                        </div>
                        <div className="flex min-w-0 items-baseline justify-between gap-3 rounded-lg bg-slate-900 p-3">
                            <p className="shrink-0 text-slate-400">익일 고가</p>
                            <p className={`min-w-0 flex-1 text-right text-lg font-semibold whitespace-nowrap tabular-nums ${getRateTextColorClass(nextHighRate)}`}>
                                {preview.nextHigh.toLocaleString()}원 ({getRateIndicator(nextHighRate)} {formatRate(nextHighRate)})
                            </p>
                        </div>
                        <div className="flex min-w-0 items-baseline justify-between gap-3 rounded-lg bg-slate-900 p-3">
                            <p className="shrink-0 text-slate-400">익일 종가</p>
                            <p className={`min-w-0 flex-1 text-right text-lg font-semibold whitespace-nowrap tabular-nums ${getRateTextColorClass(nextCloseRate)}`}>
